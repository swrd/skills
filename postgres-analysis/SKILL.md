...
4. **输出目录**：`C:/Users/用户名/Desktop/markdown/postgresql/`
...
8. **源码证据纪律（强制）**：
   - **行为声明必须有源码引用**：任何关于运行时行为的声明（"经过 X"、"使用 Y 机制"、"不经过 Z"）必须附带 `src/path/file.c:行号` 引用，否则视为无支撑断言
   - **追踪到分支决策点**：对涉及 I/O 路径、数据流向、资源分配的问题，必须追踪调用链到**分支决策点**（if/switch 分支、函数指针分发、宏判断），不得在上层接口停止
   - **负面验证**：对每个关键结论，主动搜索可能推翻该结论的代码路径（如结论为"使用 A 机制"，则 Grep 搜索 B 机制的存在性）
   - **区分引用层级**：
     - L1 引用（强）：直接读取实现代码，标注文件:行号
     - L2 引用（中）：README/SGML 文档描述
     - L3 引用（弱）：DeepWiki 或外部资料，需本地代码二次确认
     - 文档中的关键行为声明**必须至少有 L1 引用**
